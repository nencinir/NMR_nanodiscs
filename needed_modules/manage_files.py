...
#added 18.10.2022
def remove_water(folder_path,save_part,xtc):
    """
        ...
        if not xtc:
            ...
            conversions={"xtc":"echo 'converting tpr and gro files'",
                         "tpr":"echo "+save_part+"|gmx convert-tpr -s " + folder_path+"/"+content["FILES"]["tpr"]["NAME"] + " -o " 
                                ...
                                + folder_path+"/"+content["FILES"]["ndx"]["NAME"] + " >& /dev/null",
                         "gro":"echo System| gmx trjconv -f " + folder_path+"/"+content["FILES"]["xtc"]["NAME"] + 
                                ...
                                + content["FILES"]["gro"]["NAME"] + " >& /dev/null" }
        else:
...
def load_yaml_files(output_path_relax,output_path_timescales):
    ...
    """
    #TODO: there is probably no point in saving _relax.yaml as it does not take much time to 
    #calculate SRT from timescales, and this can be freely performed at different fileds...
